...
1. Read the project-memory file, if one exists. Try in order and stop at the first hit: `CLAUDE.md`, `AGENTS.md`, `GEMINI.md`, `README.md` (root, GUS/). If it lacks context, also check `app/README.md`. This adapts the skill to whichever agent is running it (Claude Code, Codex, Gemini CLI, etc.).
2. List the contents of `docs/specs/` to see which specs already exist and how they are numbered.
3. If previous specs exist, read at least the two most recent ones to pick up the project's conventions.
4. Check whether the project source code lives in a subfolder (e.g. `app/`) rather than the repo root. If so, all file paths mentioned later in the spec (data model, implementation plan) must be written relative to the repo root, including that prefix (e.g. `app/src/levels.ts`, not `src/levels.ts`).
...
4. **Implementation plan** (numbered steps, each leaving the system functional). Every file path must be written relative to the repo root — if the source code lives under a subfolder (e.g. `app/`), include that prefix explicitly (`app/src/levels.ts`).
...
1. Determine the next sequential number by looking at `docs/specs/`. If the last one is `02-powerups.md`, this one will be `03-`.
...
4. Create the file at `docs/specs/NN-slug.md` with all approved sections.
