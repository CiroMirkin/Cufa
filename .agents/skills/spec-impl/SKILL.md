...
!`ls docs/specs/ 2>/dev/null || echo "The docs/specs/ folder does not exist"`
...
- List the files available in `docs/specs/` (you already have them above).
- Ask the user to specify the exact name of the spec.
- Stop and wait for an answer. Do not continue.

If `$ARGUMENTS` has a value:

- Look for the file in `docs/specs/`. The user may have written the full name (`01-mvp-arkanoid`), only the number (`01`), or only the slug (`mvp-arkanoid`). Try to find the correct file in any of those cases.
...
/impl-spec 01-mvp-arkanoid
  ...
  Phase 1  →  Finds docs/specs/01-mvp-arkanoid.md
...
/impl-spec 02-powerups  (state: Draft / Borrador)
  ...
  Phase 1  →  Finds docs/specs/02-powerups.md
